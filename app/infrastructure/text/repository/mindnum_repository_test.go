...
func Test_mindnumRepository_FindByNumber(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: &mindnumDomain.Mindnum{
				MindNumber: 1,
				Description: `You are a CHALLENGER.
...
`,
		...
		},
		{
			...
			want: &mindnumDomain.Mindnum{
				MindNumber: 2,
				Description: `You are a MAGICIAN.
...
`,
		...
		},
		{
			...
			want: &mindnumDomain.Mindnum{
				MindNumber: 6,
				Description: `You are a MESSENGER.
...
`,
		...
		{
			name: "negative case (mindnumDomain.Embedded.ReadFile returns error)",
			fields: fields{
				DescriptionPath: "test",
			},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := &mindnumRepository{
				DescriptionPath: tt.fields.DescriptionPath,
			}
